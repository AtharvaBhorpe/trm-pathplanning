"""Core building blocks: RMSNorm and SwiGLU (complete — same as your case study)."""
...
class RMSNorm(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        rms = torch.sqrt(x.pow(2).mean(dim=-1, keepdim=True) + self.eps)  # dim: (batch_size, seq_len, feature_dim) -> (batch_size, seq_len, 1)
...
class SwiGLU(nn.Module):
    ...
    def __init__(self, dim_in: int, hidden_dim: Optional[int] = None):
        ...
        self.w1 = nn.Linear(dim_in, hidden_dim, bias=False)  # linear layer for the first part of the gate
        self.w2 = nn.Linear(dim_in, hidden_dim, bias=False)  # linear layer for the second part of the gate
        self.w3 = nn.Linear(hidden_dim, dim_in, bias=False)  # linear layer for the output projection

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # Compute the gated activation using the SwiGLU formula
        return self.w3(F.silu(self.w1(x)) * self.w2(x))  # dim: (batch_size, seq_len, feature_dim)